...
@pytest.fixture(params=["JST", "GMT", "PST"]) # Provide a list of timezones
...
def app_timezone_set_to_system_timezone(window, system_timezone, preselected_timezone):
...
def timezone_menu_visible_and_display(window, preselected_timezone):
    ...
    window.set_timezone(preselected_timezone)
    assert window.menu.currentText() == preselected_timezone
...
def user_selects_timezone(window, preselected_timezone, qtbot):
